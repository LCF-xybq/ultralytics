"""
plant_height_v3.py - Plant height inference and post-processing pipeline
...
Usage:
  python plant_height_v3.py --weight best.pt --data ./images --save_json ./results
  python plant_height_v3.py --weight best.pt --data ./images --save_json ./results --point_correct
"""
...
def point_correct(data_dir, json_dir, max_workers, kernel_size, clean_dir=""):
    ...
    inter_dirs = [rescale_dir, ruler_dir, cycle_dir, restored_dir]
    ...
    try:
        ...
        print("  2c. Refine keypoints using gradient magnitude")
        os.makedirs(cycle_dir, exist_ok=True)
        ruler_imgs = [n for n in os.listdir(ruler_dir) if osp.splitext(n)[1].lower() in IMG_EXTS]
        if ruler_imgs:
            tasks = [(ruler_dir, rescale_dir, cycle_dir, n) for n in ruler_imgs]
            with ProcessPoolExecutor(max_workers=max_workers) as ex:
                results = list(ex.map(_refine_one, tasks))
            print(f"    ok={sum(1 for ok, _, _ in results if ok)}")

        print("  2d. Restore points to original coords")
        ...
        if clean_dir:
            ...
            for d in inter_dirs:
                if osp.exists(d):
                    dst = osp.join(clean_dir, osp.basename(d))
                    if osp.exists(dst):
                        shutil.rmtree(dst)
                    shutil.move(d, dst)
